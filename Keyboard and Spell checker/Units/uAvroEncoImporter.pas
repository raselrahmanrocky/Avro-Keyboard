...
begin
  ...
  try
    ...
    begin
      ...
      begin
        ErrMsg := 'Invalid password. Please try again. (' + IntToStr(Attempt) + ' of ' + IntToStr(MAX_PASSWORD_ATTEMPTS) + ' attempts)';
        Application.MessageBox(PChar(ErrMsg), 'Password Error', MB_ICONERROR or MB_OK or MB_TOPMOST or MB_SETFOREGROUND);
      end
      else
      begin
        ErrMsg := 'Invalid password after ' + IntToStr(MAX_PASSWORD_ATTEMPTS) + ' attempts. Import cancelled.';
        Application.MessageBox(PChar(ErrMsg), 'Import Cancelled', MB_ICONERROR or MB_OK or MB_TOPMOST or MB_SETFOREGROUND);
      end;
    end;
  except
...
begin
  ...
  try
    ...
    if (ProtectionFlag <> AVROENCO_FLAG_DEFAULT_KEY) and (ProtectionFlag <> AVROENCO_FLAG_USER_PASSWORD) then
    begin
      AErrorMessage := 'Invalid .AvroEnco file header.';
      Exit;
    end;
    ...
    begin
      if Application.MessageBox(PChar('A mapping named "' + DisplayName + '" already exists.' + sLineBreak + 'Overwrite the existing file?'),
        'Confirm Overwrite', MB_ICONQUESTION or MB_YESNO or MB_DEFBUTTON2) <> ID_YES then
      begin
    ...
    begin
      AErrorMessage := 'Failed to copy file to target directory. Error: ' + SysErrorMessage(GetLastError);
      Exit;
    end;
...
procedure DoImportFileList(const AFileNames: TStrings; out AImported: Integer; out AErrorMessages: string);
var
  I:      Integer;
  ErrMsg: string;
begin
  ...
  try
    ...
    begin
      ...
      if ErrorMessages <> '' then
        Application.MessageBox(PChar(ErrorMessages), 'Import Errors', MB_ICONWARNING or MB_OK);
    end;
  finally
    FileNames.Free;
    OpenDialog.Free;
