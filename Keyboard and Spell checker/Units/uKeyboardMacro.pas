...
begin
  ...
  begin
    if (UserHotkeys[I].VirtualKey = vkCode) and (UserHotkeys[I].ModifierMask = CurrentMods) then
    begin
...
begin
  Upper := UpperCase(S);
  if Upper = 'BACKSPACE' then
    Result := $08
  else if Upper = 'CAPSLOCK' then
    Result := $14
  else if Upper = 'SPACE' then
    Result := $20
  else if Upper = 'MENU' then
    Result := $5D
  else if Upper = 'ENTER' then
    Result := $0D
    ...
    Result := $2E
  else if Upper = 'INSERT' then
    Result := $2D
  else if Upper = 'HOME' then
    Result := $24
  else if Upper = 'END' then
    Result := $23
    ...
    Result := Ord(Upper[1])
  else if Upper = 'F1' then
    Result := $70
  else if Upper = 'F2' then
    Result := $71
  else if Upper = 'F3' then
    Result := $72
  else if Upper = 'F4' then
    Result := $73
  else if Upper = 'F5' then
    Result := $74
  else if Upper = 'F6' then
    Result := $75
  else if Upper = 'F7' then
    Result := $76
  else if Upper = 'F8' then
    Result := $77
  else if Upper = 'F9' then
    Result := $78
  else if Upper = 'F10' then
    Result := $79
  else if Upper = 'F11' then
    Result := $7A
  else if Upper = 'F12' then
    Result := $7B
  else if Upper = ';' then
    Result := $BA
  else if Upper = '=' then
...
begin
  ...
  Result := ModifierMaskToStr(UserHotkeys[index].ModifierMask) + '+' + VirtualKeyToStr(UserHotkeys[index].VirtualKey) + '  ->  ' +
...
function HotkeysEqual(const A, B: string): Boolean;
begin
  Result := (HotkeyStringToModifiers(A) = HotkeyStringToModifiers(B)) and (HotkeyStringToKey(A) = HotkeyStringToKey(B));
end;
...
begin
  ...
  begin
    ...
    if (EditTxt <> '') and (EditTxt <> 'NONE') and (EditTxt <> 'PRESS ANY KEY...') and HotkeysEqual(UpperHotkey, EditTxt) then
    begin
      Result := I;
      Exit;
    end;
  ...
  for I := 0 to Length(UserHotkeys) - 1 do
  begin
    if HotkeysEqual(UpperHotkey, UpperCase(KeyToHotkeyString(UserHotkeys[I].ModifierMask, UserHotkeys[I].VirtualKey))) then
    begin
...
begin
  ...
  Msg := 'The hotkey "' + NewHotkey + '" conflicts with the hotkey selected for "' + FeatureName + '". ' + 'If you continue, the hotkey for "' + FeatureName +
    '" will be cleared. Continue?';
  Ret := Application.MessageBox(PChar(Msg), 'Hotkey Conflict', MB_YESNOCANCEL or MB_ICONWARNING or MB_DEFBUTTON3);
  if Ret = IDYES then
    Result := True;
end;
