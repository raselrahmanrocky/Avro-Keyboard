...
procedure CheckEnc(const AName: string; const APlainHex, AKeyHex,
  ANonceHex, AAADHex, AExpectedHex: string);
var
  OutB: TBytes;
  Got: string;
begin
  AES256GCMEncrypt(FromHex(APlainHex), FromHex(AKeyHex),
    FromHex(ANonceHex), FromHex(AAADHex), OutB);
  Got := HexOf(OutB);
...
procedure CheckECB(const AName: string; const AInHex, AKeyHex,
  AExpectedHex: string);
var
  OutB: TBytes;
  Got: string;
...
procedure CheckRoundtrip(const AName: string; const APlainHex, AKeyHex,
  ANonceHex, AAADHex: string);
var
  Enc, Dec: TBytes;
  Got: string;
begin
  AES256GCMEncrypt(FromHex(APlainHex), FromHex(AKeyHex),
    FromHex(ANonceHex), FromHex(AAADHex), Enc);
  if not AES256GCMDecrypt(Enc, FromHex(AKeyHex), FromHex(ANonceHex),
    FromHex(AAADHex), Dec) then
  begin
...
procedure CheckReject(const AName: string; const APlainHex, AKeyHex,
  ANonceHex, AAADHex: string);
var
  Enc, Bad: TBytes;
  Dec: TBytes;
...
begin
  ...
  begin
    ...
    if AES256GCMDecrypt(Bad, FromHex(AKeyHex), FromHex(ANonceHex),
      FromHex(AAADHex), Dec) then
      Inc(Variants)
    else
      Dec := nil;
  ...
  // 3) Wrong key.
  if AES256GCMDecrypt(Enc, FromHex('ffffffffffffffffffffffffffffffff' +
    'ffffffffffffffffffffffffffffffff'), FromHex(ANonceHex),
    FromHex(AAADHex), Dec) then
    Inc(Variants)
  else
    Dec := nil;
  ...
  // 4) Wrong AAD.
  if AES256GCMDecrypt(Enc, FromHex(AKeyHex), FromHex(ANonceHex),
    FromHex('00'), Dec) then
    Inc(Variants)
  else
    Dec := nil;
  ...
  // 5) Wrong nonce.
  if AES256GCMDecrypt(Enc, FromHex(AKeyHex),
    FromHex('ffffffffffffffffffffffff'), FromHex(AAADHex), Dec) then
    Inc(Variants)
  else
    Dec := nil;
  ...
  begin
    WriteLn('FAIL ' + AName + ' (' + IntToStr(Variants) +
      ' tamper variant(s) accepted)');
    Inc(Fails);
  end;
end;
...
begin
  ...
  CheckECB('AES-ECB single block', '000102030405060708090a0b0c0d0e0f', KEY32,
    '5a6e045708fb7196f02e553d02c3a692');
  CheckECB('AES-ECB two blocks', '000102030405060708090a0b0c0d0e0f' +
    '101112131415161718191a1b1c1d1e1f', KEY32,
    '5a6e045708fb7196f02e553d02c3a692e9c3ef8ab23453e6f0749cd636e7a88e');

  // ---- AES-256-GCM vectors (Python AESGCM, same inputs) ----
  CheckEnc('GCM empty plaintext', '', KEY32, NONCE12, '',
    'f4c2db1dc38805a37b92171c5d0a81cc');
  CheckEnc('GCM short plaintext', '00010203040506070809', KEY32, NONCE12, '',
    '4703d418c1e0c41c85488fce6c32cd364c4f746e680fe77a8444');
  CheckEnc('GCM 48 bytes + AAD', '00000000000000000000000000000000' +
    '00000000000000000000000000000000' + '00000000000000000000000000000000',
    KEY32, NONCE12, 'deadbeefcafebabe',
    '4702d61bc5e5c21b8d41978bb1e9786d83d68734f07b5f7c3867e5851d6900b2' +
    '0110aefcafc1129874a47fed8887283842a10360dfa0c13c6bb28f613e069b04');
  CheckEnc('GCM 16-byte nonce', '00010203', KEY32, NONCE16, '',
    '676da2750c835f4427c41d14b4e3b7e2a824fc05');

  // ---- Round trips across sizes / nonce lengths ----
  CheckRoundtrip('GCM roundtrip empty', '', KEY32, NONCE12, '');
  CheckRoundtrip('GCM roundtrip 48B + AAD',
    '00000000000000000000000000000000' +
    '00000000000000000000000000000000' +
    '00000000000000000000000000000000',
    KEY32, NONCE12, 'deadbeefcafebabe');
  CheckRoundtrip('GCM roundtrip nonce16 + AAD', '00010203040506070809',
    KEY32, NONCE16, 'a1b2c3d4');
  ...
  Got := '';
  if AES256GCMDecrypt(E1K, FromHex(KEY32), FromHex(NONCE12), FromHex(A), P1K)
    and (Length(P1K) = 1024) then
  begin
    Got := 'ok';
    for I := 0 to 1023 do
  ...
  CheckReject('GCM rejects tampering', '00010203040506070809', KEY32,
    NONCE12, 'deadbeef');
