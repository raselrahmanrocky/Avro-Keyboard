...
begin
  ...
  for I := 1 to N do
    if A[I] <> B[I] then
    begin
      Result := Format('at %d: live=%s golden=%s', [I, Copy(A, I, 12), Copy(B, I, 12)]);
      Exit;
    end;
...
begin
  Result := TArray < TPair < string, string >>.Create(TPair<string, string>.Create('aa-kar', Chr(U_AAKAR)), TPair<string, string>.Create('i-kar', Chr(U_IKAR)),
    TPair<string, string>.Create('ii-kar', Chr(U_IIKAR)), TPair<string, string>.Create('u-kar', Chr(U_UKAR)),
    TPair<string, string>.Create('uu-kar', Chr(U_UUKAR)), TPair<string, string>.Create('ri-kar', Chr(U_RIKAR)),
    TPair<string, string>.Create('e-kar', Chr(U_EKAR)), TPair<string, string>.Create('oi-kar', Chr(U_OIKAR)), TPair<string, string>.Create('o-kar', Chr(U_OKAR)
...
begin
  ...
  R_ := Chr(U_R);
  S_ := Chr(U_S);
  Result := TArray<string>.Create(K, K + Chr(U_AAKAR), K + Chr(U_IKAR), K + Chr(U_IIKAR), K + Chr(U_UKAR), K + Chr(U_UUKAR), K + Chr(U_RIKAR), K + Chr(U_EKAR),
    K + Chr(U_OIKAR), K + Chr(U_OKAR), K + Chr(U_OUKAR), K + Chr(U_HASANTA) + Chr(U_AAKAR), K + Chr(U_HASANTA) + T_, K + Chr(U_HASANTA) + R_,
    K + Chr(U_HASANTA) + S_, K + Chr(U_ANUSVARA), K + Chr(U_CHANDRABINDU), R_ + Chr(U_HASANTA) + K, K + Chr(U_HASANTA) + T_ + Chr(U_HASANTA) + R_);
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        if Conv.ResolveAnsiSequence(Chr(U_K), Kar.Value, Isolated, Erase, Matched, IsToggle, UsedAlt) then
...
begin
  ...
  begin
    ...
    Results[I].ErrorMsg := '';
    try
      Results[I].OK := LoadAnsiJSONCached(Items[I].FilePath, Items[I].Password, Results[I].JSON) and (Results[I].JSON <> '') and (Results[I].JSON[1] = '{');
    except
      on E: Exception do
...
begin
  ...
  Check(AnsiEngineManager.CurrentEngineName = Lowercase(AName), AWhere + ': active engine is "' + AName + '" (got "' +
  ...
  Got := Fingerprint;
  if Got = Want then
    Check(True, AWhere + ': live engine behaves exactly like "' + GoldenNameOf(AName) + '"')
  else
    Check(False, AWhere + ': live engine differs from "' + GoldenNameOf(AName) + '" (' + FirstDiff(Got, Want) + ')');
end;

function FreshDir(const AName: string): string;
begin
  Result := IncludeTrailingPathDelimiter(TPath.Combine(TPath.GetTempPath, AName));
...
begin
  ...
  try
    ...
    Check(Count >= 2, 'at least two shippable containers in ' + Dir + ' (found ' + IntToStr(Count) + ')');
    if Count < 2 then
    begin
    ...
    Check(AnsiEngineManager.WarmEngineCount <= MaxWarmEngines, '1: the switch trims the batch to the warm limit (warm=' +
    ...
    Check(AnsiEngineManager.TrySwitchCached(TAG_COLD_B), '3: clicking the already-active version reports success');
    ...
    try
      Check(not AnsiEngineManager.SwitchEngine(TAG_COLD_BAD, ErrLog), '6: switching to a corrupt container fails');
    ...
    Check(not AnsiEngineManager.TrySwitchCached(Prev), '7: fast switch does not claim success for a hollow engine');
    ...
    try
      ...
      Check(CompareMappingDisplayNames('ansi v1', 'Ansi V1') = 0, '9: comparison ignores case');
      Check(CompareMappingDisplayNames('Ansi V1', 'Ansi V10') < 0, '9: a prefix sorts before its extension');
      Check(CompareMappingDisplayNames('Ansi V01', 'Ansi V1') = 0, '9: leading zeros do not change the number');
      Check(CompareMappingDisplayNames('Ansi V3', 'Ansi V10') < 0, '9: 3 still sorts before 10');
    ...
    Check(ResolveAppTheme(atmSystemDefault, True) = atmLight, '10: SYSTEM follows Windows when Windows uses light mode');
    Check(ResolveAppTheme(atmSystemDefault, False) = atmDark, '10: SYSTEM follows Windows when Windows uses dark mode');
    Check(ResolveAppTheme(atmLight, False) = atmLight, '10: forced LIGHT wins over a dark Windows');
    Check(ResolveAppTheme(atmDark, True) = atmDark, '10: forced DARK wins over a light Windows');
    ...
    Check(AppThemeModeFromSetting('dark') = atmDark, '10: the stored setting is case-insensitive');
    Check(AppThemeModeFromSetting('') = atmSystemDefault, '10: empty setting means SYSTEM');
    Check(AppThemeModeFromSetting('TWILIGHT') = atmSystemDefault, '10: an unknown setting falls back to SYSTEM');
    Check(AppThemeModeToSetting(atmSystemDefault) = APP_THEME_SETTING_SYSTEM, '10: SYSTEM setting round trip');
    Check(AppThemeModeToSetting(atmLight) = APP_THEME_SETTING_LIGHT, '10: LIGHT setting round trip');
    Check(AppThemeModeToSetting(atmDark) = APP_THEME_SETTING_DARK, '10: DARK setting round trip');
    ...
    Check(PaletteDark.IsDark and (not PaletteLight.IsDark), '10: each palette reports its own brightness');
    Check((PaletteDark.Background = RGB(32, 32, 32)) and (PaletteDark.Text = RGB(240, 240, 240)) and (PaletteDark.SelectionFill = RGB(0, 120, 215)) and
        (PaletteDark.SelectionText = RGB(255, 255, 255)) and (PaletteDark.Border = RGB(60, 60, 60)), '10: dark palette matches the documented colours');
    Check((PaletteLight.Background = RGB(255, 255, 255)) and (PaletteLight.Text = RGB(0, 0, 0)) and (PaletteLight.SelectionFill = RGB(0, 120, 215)) and
        (PaletteLight.SelectionText = RGB(255, 255, 255)) and (PaletteLight.Border = RGB(200, 200, 200)), '10: light palette matches the documented colours');
    Check(PaletteDark.HoverFill <> PaletteLight.HoverFill, '10: the row hover tint differs per theme');
    Check(not GetAppThemePalette(atmSystemDefault).IsDark, '10: an unresolved mode paints light, never an unstyled window');
    ...
    if (DfmPath = '') or (PasPath = '') then
      Say('SKIP: uForm1.dfm / uForm1.pas not reachable from ' + ExtractFilePath(ParamStr(0)))
    else
    begin
      ...
      try
        ...
        for I := 0 to DfmText.Count - 1 do
          if (LeadingSpaces(DfmText[I]) = 2) and SameText(DfmObjectName(DfmText[I]), 'Popup_Tray') then
          begin
            TrayIdx := I;
            Break;
          end;
        ...
        begin
          ...
          Check(AnsiIdx >= 0, '11: mnuTraySelectAnsiEncoding is a direct child of Popup_Tray');
          Check((LayoutIdx >= 0) and (AnsiIdx = LayoutIdx + 1), '11: it sits directly beneath "Select keyboard layout" (index ' + IntToStr(LayoutIdx) + ' -> ' +
              IntToStr(AnsiIdx) + ')');
          ...
          ItemIdx := -1;
          for I := TrayIdx + 1 to DfmText.Count - 1 do
            if (LeadingSpaces(DfmText[I]) = 4) and SameText(DfmObjectName(DfmText[I]), 'mnuTraySelectAnsiEncoding') then
            begin
              ItemIdx := I;
              Break;
            end;
          ...
          Check(CaptionLine = 'Caption = ' + QuotedStr('Select ANSI Encoding'), '11: its caption is exactly "Select ANSI Encoding" (found "' +
              CaptionLine + '")');
          Check(not HasNestedObject, '11: the DFM holds no second copy of the version list - no child items');
          ...
          Check(LinesContain(PasText, 'mnuTraySelectAnsiEncoding: TMenuItem;'), '11: the DFM object has a matching published field');
        end;

        // The three shared routines the two existing ANSI menus already go
        // through; the tray item has to be in all of them.
        Check(LinesContain(PasText, 'BuildSingleMenu(mnuTraySelectAnsiEncoding)'), '11: BuildAnsiVersionMenus populates it with the sorted list');
        Check(LinesContain(PasText, 'UpdateOne(mnuTraySelectAnsiEncoding)'), '11: UpdateAnsiVersionMenuChecks marks the active version in it');
        Check(LinesContain(PasText, 'SyncAnsiVersionChecks(mnuTraySelectAnsiEncoding)'), '11: PopupTrayPopup re-syncs its checkmark on every popup');
        ...
        ParentBody := RoutineBody(PasText, 'procedure TAvroMainForm1.ReplaceAnsiMenuParentIcon;');
        Check(ParentBody <> '', '11: ReplaceAnsiMenuParentIcon is present');
        Check(Pos('mnuTraySelectAnsiEncoding.ImageIndex := Slot', ParentBody) > 0, '11: the tray parent item gets the active layout badge');
        Check(Pos('AnsiVersionSubmenu1.ImageIndex := Slot', ParentBody) > 0, '11: the TopBar tools parent item gets the SAME badge');
        Check(Pos('AnsiRootIconSlot(', ParentBody) > 0, '11: and it is resolved through AnsiRootIconSlot');
        Check(Pos('ANSI_ROOT_IMAGE_INDEX = 30;', ParentBody) > 0, '11: the built-in ANSI icon stays the named fallback at slot 30');
        Check(Pos('OutputIsBijoy', ParentBody) = 0, '11: the badge follows the selection like the checkmark, not the output mode');
        Check(Pos('GetAnsiTrayIcon', ParentBody) = 0, '11: the DPI-scaled tray handle is not what the parent badge is built from');

        SlotBody := RoutineBody(PasText, 'function TAvroMainForm1.AnsiRootIconSlot(');
        Check(SlotBody <> '', '11: AnsiRootIconSlot is present');
        Check(Pos('GetMappingIconBytes(AName)', SlotBody) > 0, '11: it resolves the one icon key the submenu badges use');
        Check(Pos('CreateHIconAtSize(IconBytes, Cols, Rows)', SlotBody) > 0, '11: the frame is decoded at ImageList1''s own metric');
        Check(Pos('ImageList_ReplaceIcon', SlotBody) > 0, '11: an appended slot is overwritten in place, not appended again');
        Check(Pos('ExtractFileName', SlotBody) = 0, '11: no second key derivation (case/space/name guessing) was added');
        Check(not LinesContain(PasText, 'EnsureAnsiIconIndex'), '11: the orphaned second icon cache from the previous refactor is gone');
        Check(not LinesContain(PasText, 'AnsiIconIndexes'), '11: ...and so is its index dictionary');

        ToolsBody := RoutineBody(PasText, 'procedure TAvroMainForm1.PopupToolsPopup(');
        Check(Pos('ReplaceAnsiMenuParentIcon', ToolsBody) > 0, '11: the TopBar tools popup refreshes the parent badge on every popup');

        TrayBody := RoutineBody(PasText, 'procedure TAvroMainForm1.UpdateTrayIcon;');
        SplitPos := Pos('IsFormVisible(''TopBar'')', TrayBody);
        Check((SplitPos > 0) and (Pos('ReplaceAnsiMenuParentIcon', TrayBody) > 0) and (Pos('ReplaceAnsiMenuParentIcon', TrayBody) < SplitPos),
          '11: UpdateTrayIcon refreshes the badge BEFORE the tray/TopBar split');
        Check(Pos('ReplaceAnsiMenuParentIcon', Copy(TrayBody, SplitPos)) = 0, '11: ...so the TopBar branch refreshes it too, and never twice');
      finally
        TrayChildren.Free;
        PasText.Free;
        DfmText.Free;
