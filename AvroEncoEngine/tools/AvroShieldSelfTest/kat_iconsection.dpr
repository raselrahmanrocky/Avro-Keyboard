...
const
  ...
  PipelineJson =
    '{"Metadata":{"Encoding":"KatIcon","Suggested Font":"X"},'
    + ' "FullFormReplacements":[], "Constants":{}, "PreReplacements":[],'
    + ' "PostReplacements":[]}';
...
begin
  ...
  Check('synthetic icon: the frame table is read', Length(Frames) = 2,
    'frames=' + IntToStr(Length(Frames)));
  Check('synthetic icon: extents are decoded', (Length(Frames) = 2) and
    (Frames[0].Width = 16) and (Frames[1].Width = 32), SizesOfText(Frames));

  Check('malformed icon: an empty blob has no frames',
    Length(IconFramesOf(nil)) = 0);
  // NOTE: Copy() is 0-based for dynamic arrays (unlike strings), so every
  // slice below counts from 0.
  Check('malformed icon: a truncated header has no frames',
    Length(IconFramesOf(Copy(Ico, 0, 4))) = 0);
  Check('malformed icon: the reserved word must be zero',
    Length(IconFramesOf(CorruptReserved(Ico))) = 0);
  ...
  Check('malformed icon: a frame pointing past EOF drops the table',
    Length(IconFramesOf(Corrupt)) = 0);

  // Exact match wins: the 100% DPI tray case and every menu draw.
  Check('selection: 16 px resolves to the 16 px frame',
    PickBestIconFrame(Ico, 16, 16, Frame, W, H) and (W = 16) and (H = 16) and
    (FrameTag(Frame) = TAG_16),
    Format('w=%d tag=%.2x', [W, FrameTag(Frame)]));
  Check('selection: 32 px resolves to the 32 px frame',
    PickBestIconFrame(Ico, 32, 32, Frame, W, H) and (W = 32) and
    (FrameTag(Frame) = TAG_32),
    Format('w=%d tag=%.2x', [W, FrameTag(Frame)]));
  ...
  Check('selection: 20 px (125%) resolves to the 32 px frame',
    PickBestIconFrame(Ico, 20, 20, Frame, W, H) and (W = 32) and
    (FrameTag(Frame) = TAG_32),
    Format('w=%d tag=%.2x', [W, FrameTag(Frame)]));
  Check('selection: 24 px (150%) resolves to the 32 px frame',
    PickBestIconFrame(Ico, 24, 24, Frame, W, H) and (W = 32) and
    (FrameTag(Frame) = TAG_32),
    Format('w=%d tag=%.2x', [W, FrameTag(Frame)]));

  // Nothing big enough: enlarge the largest rather than fail.
  Check('selection: 48 px with no large frame falls back to the largest',
    PickBestIconFrame(Ico, 48, 48, Frame, W, H) and (W = 32), Format('w=%d', [W]));
  ...
  Check('selection: a 48-only icon still resolves 16 px',
    PickBestIconFrame(OnlyBig, 16, 16, Frame, W, H) and (W = 48));
  Check('selection: a 48-only icon still resolves 24 px',
    PickBestIconFrame(OnlyBig, 24, 24, Frame, W, H) and (W = 48));
  Check('selection: a 48-only icon still resolves 32 px',
    PickBestIconFrame(OnlyBig, 32, 32, Frame, W, H) and (W = 48));

  Check('selection: a zero target is refused',
    not PickBestIconFrame(Ico, 0, 0, Frame, W, H));
  Check('selection: a nil icon is refused',
    not PickBestIconFrame(nil, 16, 16, Frame, W, H));
  ...
  Check('packing: only the requested frames survive',
    (Length(Frames) = 2) and (Frames[0].Width = 16) and (Frames[1].Width = 32),
    SizesOfText(Frames));
  Check('packing: every frame stays inside AVRO_ICON_MAX_FRAME_SIZE',
    (Length(Frames) = 2) and (Frames[0].Width <= AVRO_ICON_MAX_FRAME_SIZE) and
    (Frames[1].Width <= AVRO_ICON_MAX_FRAME_SIZE));
  Check('packing: the 48 px frame is not carried by a 16+32 pack',
    (Length(IconFramesOf(Repacked)) = 2));
  Check('packing: asking for an absent size yields nothing',
    Length(BuildIconFromFrames(Ico, [96])) = 0);
  Check('packing: a nil icon yields nothing',
    Length(BuildIconFromFrames(nil, [16])) = 0);

  // Byte stability is what makes a build reproducible and lets the container
  // check below assert an idempotent round trip.
  Check('packing: re-packing the packed icon is byte-identical',
    (Length(Repacked) > 0) and
    (Hex(BuildIconFromFrames(Repacked, [16, 32])) = Hex(Repacked)));
  ...
  Check('PNG frames: the frame table still reads the PNG icon',
    Length(Frames) = 1);
  if Length(Frames) = 1 then
  begin
    var PngFrame: TBytes := Copy(Png, Frames[0].DataOffset, Frames[0].DataSize);
    Check('PNG frames: a PNG frame is detected',
      IsPortableNetworkFrame(PngFrame), Format('frameLen=%d first=%.2x',
        [Length(PngFrame), PngFrame[0]]));
  end
  else
    Check('PNG frames: a PNG frame is detected', False,
      'the frame table could not be read');

  Frames := IconFramesOf(Ico);
  Check('PNG frames: the extracted 16 px frame is the tagged frame',
    (Length(Frames) = 2) and
    (FrameTag(Copy(Ico, Frames[0].DataOffset, Frames[0].DataSize)) = TAG_16));
  Check('PNG frames: a BMP frame is not mistaken for PNG',
    (Length(Frames) = 2) and
    (not IsPortableNetworkFrame(Copy(Ico, Frames[0].DataOffset,
      Frames[0].DataSize))));
  Check('PNG frames: a PNG-only icon refuses to produce a frame',
    not PickBestIconFrame(Png, 16, 16, Frame, W, H));
  Check('PNG frames: a PNG-only icon yields no HICON',
    CreateHIconAtSize(Png, 16, 16) = 0);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      if IconBitmapExtent(H, W, Cy) then
        Check(Format('HICON: the %d px metric yields a %d px handle',
          [DPI_METRICS[I], DPI_METRICS[I]]), (W = DPI_METRICS[I]) and
          (Cy = DPI_METRICS[I]), Format('handle is %dx%d', [W, Cy]))
      else
        Check(Format('HICON: the %d px handle can be measured', [DPI_METRICS[I]]),
          False, 'GetIconInfo/GetObject failed');
    finally
      if H <> 0 then
        DestroyIcon(H);
    end;
  ...
  H := CreateHIconAtSize(Ico, GetSystemMetrics(SM_CXSMICON),
    GetSystemMetrics(SM_CYSMICON));
  Check(Format('HICON: the live SM_CXSMICON metric (%d px) is reachable',
    [GetSystemMetrics(SM_CXSMICON)]), H <> 0);
  if H <> 0 then
    DestroyIcon(H);
...
var
  ...
  Err: string;
begin
  Check('--icon-sizes: "16,32" parses',
    ParseIconSizes('16,32', Sizes, Err) and (Length(Sizes) = 2) and
    (Sizes[0] = 16) and (Sizes[1] = 32), Err);
  Check('--icon-sizes: " 16 ; 32 " tolerates spaces and semicolons',
    ParseIconSizes(' 16 ; 32 ', Sizes, Err) and (Length(Sizes) = 2), Err);
  Check('--icon-sizes: an empty list is refused',
    not ParseIconSizes('', Sizes, Err));
  Check('--icon-sizes: a non-number is refused',
    not ParseIconSizes('16,abc', Sizes, Err));
  Check('--icon-sizes: 0 is refused', not ParseIconSizes('0,16', Sizes, Err));
  Check('--icon-sizes: a value above the bound is refused',
    not ParseIconSizes(IntToStr(AVRO_ICON_MAX_FRAME_SIZE + 1), Sizes, Err));
  Check('--icon-sizes: the bound itself is accepted',
    ParseIconSizes(IntToStr(AVRO_ICON_MAX_FRAME_SIZE), Sizes, Err), Err);
  ...
  Check('default frames: the 16 px menu/tray frame leads',
    (Length(AVRO_ICON_FRAME_SIZES) > 0) and (AVRO_ICON_FRAME_SIZES[0] = 16));
  Check('default frames: a 32 px frame covers the 200% metric',
    (Length(AVRO_ICON_FRAME_SIZES) > 1) and (AVRO_ICON_FRAME_SIZES[1] = 32));
  Check('default frames: every default is within the bound',
    (Length(AVRO_ICON_FRAME_SIZES) > 0) and
    (AVRO_ICON_FRAME_SIZES[High(AVRO_ICON_FRAME_SIZES)] <= AVRO_ICON_MAX_FRAME_SIZE));
  Check('default frames: the set is exactly what the shipped icons carry',
    Length(AVRO_ICON_FRAME_SIZES) = 2);
end;
...
begin
  ...
  Check('section: the encoded value is the down-selected .ico',
    Hex(Back) = Hex(Dropped));
  ...
  Check('section: the member is a JSON string', (Parsed is TJSONObject) and
    (TJSONObject(Parsed).GetValue(AVRO_ICON_SECTION) is TJSONString));
  Check('section: the member is not an array or object, so no section census ' +
    'can see it',
    (Parsed is TJSONObject) and
    (not (TJSONObject(Parsed).GetValue(AVRO_ICON_SECTION) is TJSONObject)) and
    (not (TJSONObject(Parsed).GetValue(AVRO_ICON_SECTION) is TJSONArray)));
  Parsed.Free;

  Back := ExtractIconSection(Doc);
  Check('section: extraction returns the exact encoded bytes',
    Hex(Back) = Base64);

  // Robustness: a mapping must never be lost to a damaged icon member.
  Check('section: a mapping without the member yields no icon',
    ExtractIconSection('{"Metadata":{}, "FullFormReplacements":[]}') = nil);
  Check('section: an empty document yields no icon', ExtractIconSection('') = nil);
  Check('section: non-JSON yields no icon',
    ExtractIconSection('not json at all') = nil);
  Check('section: a non-string member yields no icon',
    ExtractIconSection('{"' + AVRO_ICON_SECTION + '": 42}') = nil);
  Check('section: damaged Base64 yields no icon',
    ExtractIconSection('{"' + AVRO_ICON_SECTION + '": "!!!not base64!!!"}') = nil);
  Check('section: decoded bytes that are not an .ico yield no icon',
    ExtractIconSection('{"' + AVRO_ICON_SECTION + '": "QUJD"}') = nil);
end;
...
begin
  Result := False;
  R := AvroShieldBuildFromJson(AJson, APassword, False, False, False, AOut,
    nil, nil);
  Result := R = asrOk;
end;
...
begin
  ...
  Bytes := nil;
  Check('pipeline: the container builds', ShieldBuild(Doc, Password, Bytes),
    'AvroShieldBuildFromJson failed');
  ...
  if Text <> '' then
  begin
    Check('pipeline: the mapping survived the pipeline',
      Pos('KatIcon', Text) > 0);
    Loaded := ExtractIconSection(Text);
    Check('pipeline: the icon survives the whole container pipeline',
      Loaded <> nil);
    // Non-empty on both sides, so this can never pass vacuously on a missing
    // section.
    Check('pipeline: the recovered icon is byte-identical',
      (Loaded <> nil) and (Base64 <> '') and (Hex(Loaded) = Base64),
      'recovered ' + IntToStr(Length(Loaded)) + ' bytes');
    Check('pipeline: the recovered icon is a usable .ico',
      Length(IconFramesOf(Loaded)) = 2, SizesOfText(IconFramesOf(Loaded)));
    ...
    Check('pipeline: the recovered .ico re-packs byte-identically',
      (Length(Original) > 0) and (Hex(Original) = Base64));
  end;

  // Backward compatibility for every container that predates the icon.
  Bytes := nil;
  Check('compat: a container without the icon member still builds',
    ShieldBuild(PipelineJson, Password, Bytes));
  ...
  Check('compat: a container without the icon member reports no icon',
    ExtractIconSection(Text) = nil);
end;
...
begin
  ...
  begin
    ...
    Check(ShortName + ': the icon is a valid .ico', Length(Frames) > 0,
      SizesOfText(Frames));
    ...
    Check(ShortName + ': has a 16 px frame (the menu and 100% tray metric)',
      Has16, SizesOfText(Frames));
    Check(ShortName + ': no frame exceeds AVRO_ICON_MAX_FRAME_SIZE',
      not TooBig, SizesOfText(Frames));
    Note(ShortName + ': frames ' + SizesOfText(Frames) + ', ' +
      IntToStr(Length(Icon)) + ' bytes');
    ...
    Check(ShortName + ': the live SM_CXSMICON metric resolves',
      PickBestIconFrame(Icon, GetSystemMetrics(SM_CXSMICON),
        GetSystemMetrics(SM_CYSMICON), Frame, W, H));

    var Live: HICON := CreateHIconAtSize(Icon, GetSystemMetrics(SM_CXSMICON),
      GetSystemMetrics(SM_CYSMICON));
    try
      Check(ShortName + ': a tray handle can be built at the live metric',
        Live <> 0);
    finally
      if Live <> 0 then
        DestroyIcon(Live);
...
begin
  ...
  TmpDir := IncludeTrailingPathDelimiter(GetEnvironmentVariable('TEMP')) +
    'avro_iconsection_' + IntToStr(GetCurrentProcessId) + PathDelim;
