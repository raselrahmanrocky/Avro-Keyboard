  ...
  try
    ...
    try
      ...
      if PreloadThread.FatalException <> nil then
        Check('preload thread raised no exception', False, Exception(PreloadThread.FatalException).Message)
      else
    ...
    begin
      ...
      Out1 := Converter.Convert(#$0986#$09AE#$09BE#$09B0) + '|' + Converter.Convert(#$0995#$09BF);
      Check('deterministic ' + V, OutputOf[Lowercase(V)] = Out1, 'expected ' + OutputOf[Lowercase(V)] + ' got ' + Out1);
    end;
    ...
    Check('the preload batch parked more than the live engine', BatchWarm >= 2, 'the batch left ' + IntToStr(BatchWarm) + ' engine(s) parked');
    ...
    Check('the ordinary switches trimmed the batch to the warm limit', AnsiEngineManager.WarmEngineCount <= MaxWarmEngines,
      'warm=' + IntToStr(AnsiEngineManager.WarmEngineCount) + ' limit=' + IntToStr(MaxWarmEngines));
    ...
    begin
      ...
      WarmName := '';
      for V in EngineList do
        if (not SameText(V, ActiveName)) and AnsiEngineManager.CachedEngineReady(V) then
        begin
          WarmName := V;
          Break;
        end;
      ...
      begin
        ...
        WriteLn('200 warm switches took ' + IntToStr(SwitchMs) + ' ms total (' + Format('%.3f', [SwitchMs / 200.0]) + ' ms each avg)');
        ...
        Check('warm switches are ~instant', SwitchMs < 500, IntToStr(SwitchMs) + ' ms for 200 switches (' + Format('%.3f', [SwitchMs / 200.0]) + ' ms each)');

        // The LRU must have run on every one of those switches.
        Check('the warm limit is enforced by switching', AnsiEngineManager.WarmEngineCount <= MaxWarmEngines,
          'warm=' + IntToStr(AnsiEngineManager.WarmEngineCount) + ' limit=' + IntToStr(MaxWarmEngines));
      end;
      ...
      ColdName := '';
      for V in EngineList do
        if (not SameText(V, ActiveName)) and (not SameText(V, WarmName)) and (not AnsiEngineManager.CachedEngineReady(V)) then
        begin
          ColdName := V;
          Break;
        end;
      ...
      begin
        ...
        SW.Stop;
        WriteLn('cold switch to ' + ColdName + ' took ' + IntToStr(SW.ElapsedMilliseconds) + ' ms (one decrypt + parse)');
        ...
        if OutputOf.ContainsKey(Lowercase(ColdName)) then
          Check('the cold-parsed engine renders identically', OutputOf[Lowercase(ColdName)] = Converter.Convert(#$0986#$09AE#$09BE#$09B0) + '|' +
    ...
    Check('idle release is time-gated', Refused = 0, 'dropped ' + IntToStr(Refused) + ' engine(s) while the user was active');
    Check('the time-gated release left the cache alone', AnsiEngineManager.WarmEngineCount >= 1, 'warm=' + IntToStr(AnsiEngineManager.WarmEngineCount));
